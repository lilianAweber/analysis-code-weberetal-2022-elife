function dprst_pharma_erpstats_roving_phases_from_1stlevel_redN( options )
%DPRST_PHARMA_ERPSTATS_ROVING_PHASES_FROM_1STLEVEL_REDN Computes the second 
%level contrast images for ERP effects for the roving_phases design in the 
%DPRST study, with reduced sample size.
...
disp(['Computing group stats (reduced N) for drug effects on ERPs in ' ...
...
for iContrast = 1: numel(contrastsOfInterest)
    facdir = fullfile(paths.pharmastatserpredn, num2str(iContrast));
    if ~exist(facdir, 'dir')
        mkdir(facdir);
    end
