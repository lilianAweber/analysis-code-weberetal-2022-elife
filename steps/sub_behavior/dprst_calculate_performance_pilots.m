function behav = dprst_calculate_performance_pilots( MMN, responseButtons, options )
%DPRST_CALCULATE_PERFORMANCE_PILOTS Calculates performance score for visual
%distraction task in the DPRST study for anta pilots specifically
...
% for pilots, we only recorded the time for when a stimulus was actually
% presented. 
%idxVis = find(MMN.stimuli.visSequence);
idxVis = 1:length(MMN.stimuli.visTimes);
...
for vis = 1: nVis
    ...
    % here, we use visTypes directly for pilots (instead of visSequence)
    presType = MMN.stimuli.visTypes(idxVis(vis));
...
behav.hitrate = (36-behav.misses)/36;
behav.errorrate = behav.errors/36;
