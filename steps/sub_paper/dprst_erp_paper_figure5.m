function [ data ] = dprst_erp_paper_figure5( options, savePath )
%DPRST_ERP_PAPER_FIGURE5 Collects all images and data points needed to 
%create figure 5 of the ERP paper: mismatch effects in study 2
...
figPath = fullfile(savePath, 'figure_5_agon_mismatch');
mkdir(figPath);
...
% collect data for fig. 5A: SPM results for mismatch 
...
data.mismatch.peak2      = con.clusters(3).peak; % middle cluster (292ms)
data.mismatch.timewin2   = con.clusters(3).timewin;
data.mismatch.peak3      = con.clusters(2).peak; % late cluster (396ms)
...
% collect data for fig. 5B: ERP plots for mismatch
