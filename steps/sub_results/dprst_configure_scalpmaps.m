function [ conf ] = dprst_configure_scalpmaps(con, options, flag, maxStatValue, peakSens)
%DPRST_CONFIGURE_SCALPMAPS
%   IN:   con    -
%         options - 
%         flag    - 
%   OUT:  conf    -
...
if nargin < 4
    if ischar(flag)
        error('Please indicate the maximum value of the statistic for the colorbars.');
    end
else
    maxStatValue = ceil(maxStatValue);
    if maxStatValue/50 > 3
        tickDist = 50;
    else
        if maxStatValue/20 > 3
            tickDist = 20;
        elseif maxStatValue/10 < 4
            tickDist = 5;
        else
            tickDist = 10;
        end
...
%map = cmocean('curl',104);
%map(1:20, :) = [];
%map(end-19:end, :) = [];
%map = colormap('parula');
%map(1:7, :) = [];
map = cmocean('balance',104);
map(1:20, :) = [];
map(end-19:end, :) = [];
...
if ~isempty(peakSens)
    conf.cfg.highlight          = 'on';
    conf.cfg.highlightchannel   = peakSens;
    conf.cfg.highlightsymbol    = '.'; % (default = 'o')
    conf.cfg.highlightcolor     = [1 1 1]; % (default = [0 0 0] (black))
    conf.cfg.highlightsize      = 25; % (default = 6)
...
if ischar(flag)
    switch flag
        case 'F'
            conf.cfg.zlim = [0 maxStatValue];
            conf.colmap.ticks = [0:tickDist:tickMax];
            conf.colmap.limits = [0 maxStatValue];
        case 'T'
            conf.cfg.zlim = [-maxStatValue maxStatValue];
            conf.colmap.ticks = [-tickMax:tickDist:tickMax];
            conf.colmap.limits = [-maxStatValue maxStatValue];
    end
else
    conf.cfg.zlim = flag;
    conf.colmap.limits = conf.cfg.zlim;
    if flag(1) < 0
        conf.colmap.ticks = [-4 -2 0 2 4];
    else
        conf.colmap.ticks = [0 15 30 45];
    end
end
...
% configure colorbars
%col = colormap('parula');
%col = cmocean('curl','negative',64);
%col(1:7, :) = [];
conf.colmap.colmap = map;
...
conf.colmap.width = 0.06;
conf.colmap.filename = 'colormap_';
end
