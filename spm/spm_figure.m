...
case 'parula'
    colormap(parula(64))
case 'balance'
    balance = cmocean('balance',64);
    colormap(balance)
case 'mybalance'
    balance = cmocean('balance',104);
    balance(1:20, :) = [];
    balance(end-19:end, :) = [];
    colormap(balance)
case 'curl'
    curl = cmocean('curl',64);
    colormap(curl)
case 'mycurl' %lighter version of curl
    curl = cmocean('curl',104);
    curl(1:20, :) = [];
    curl(end-19:end, :) = [];
    colormap(curl)
case '-curl'
    curl = cmocean('curl','negative',64);
    colormap(curl)
case 'tarn'
    tarn = cmocean('tarn',64);
    colormap(tarn)
case 'gray-hot'
...
case 'gray-parula'
    colormap([gray(64); parula(64)]);
case 'gray-balance'
    balance = cmocean('balance',64);
    colormap([gray(64); balance]);    
case 'invert'
