function varargout = tnueeg_line_with_shaded_errorbar(x, y, errBar, lineProps, transparent)
%TNUEEG_LINE_WITH_SHADED_ERRORBAR(x,y,errBar,lineProps,transparent)
% Makes a 2-d line plot with a pretty shaded error bar made
% using patch. Error bar color is chosen automatically.
% IN:   x       - vector of x values [optional, can be left empty]
%       y       - vector of y values or a matrix of n observations by m 
%               cases where m has length(x);
...
%               image. 
% OUT:  H       - a structure of handles to the generated plot objects.     
%
% Examples
% y=randn(30,80); x=1:size(y,2);
% tnueeg_line_with_shaded_errorbar(x,mean(y,1),std(y),'g');
% tnueeg_line_with_shaded_errorbar(x,y,{@median,@std},{'r-o','markerfacecolor','r'});    
% tnueeg_line_with_shaded_errorbar([],y,{@median,@std},{'r-o','markerfacecolor','r'});    
%
% Overlay two transparent lines
% y=randn(30,80)*10; x=(1:size(y,2))-40;
% tnueeg_line_with_shaded_errorbar(x,y,{@mean,@std},'-r',1); 
% hold on
% y=ones(30,1)*x; y=y+0.06*y.^2+randn(size(y))*10;
% tnueeg_line_with_shaded_errorbar(x,y,{@mean,@std},'-b',1); 
% hold off
%
%
% written by Rob Campbell - November 2009
% modified by Lilian Weber July 2017
% NOTE for modifications L.W.: remove edges, modify comments
...
uE=y+errBar(1,:);  % was errBar(1,:);   %was y+errBar(1,:);
lE=y-errBar(2,:);  % was errBar(2,:);   %was y-errBar(2,:);
