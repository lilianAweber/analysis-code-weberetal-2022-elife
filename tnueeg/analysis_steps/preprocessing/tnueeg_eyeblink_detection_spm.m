...
if isfield(options.preproc, 'eyeblinkdetection')
    ...
    switch options.preproc.eyeblinkdetection
        case 'sdThresholding'
            ...
            S.methods.settings.excwin = 1; % excision window: Window (in ms) to mark as bad around each jump 
            % must be >0; exact value is irrelevant for manual rejection &
            % for correction, choose 1000 (=1s) for SPM event-based
            % rejection
            S.prefix = 'b';
    ...
    switch options.preproc.eyeblinktreatment
        ...
        case {'ssp', 'berg'}
            ...
            S.windowOfInterestInSamples = options.preproc.windowForEyeblinkdetection;        
            S.methods.settings.excwin = 1; % setting not relevant for EB correction; any value >0
